...
`price_layers_shares` — same structure as `price_layers`, but every value is divided by total use at purchasers' prices for that product and year. Expresses each price layer as a share of the product's total purchasers' price use.
