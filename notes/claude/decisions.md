...
- **2026-04-22 (session 6)**: Added `as_list: bool = False` keyword argument to `get_index_values` on all 7 inspection result classes and `TablesComparison`. When `True`, returns a plain Python list; raises `ValueError` if more than one level is requested. Logic lives in `_get_index_values` in `_shared.py`; all 8 public methods are thin delegates.

- **2026-04-22 (session 5)**: Added `supply_summary` and `use_summary` to `ProductInspection`. Index: `(product, product_txt, summary)` — one block per product. Statistics aggregate across (transaction, category) rows within each product. `n_categories` / `n_categories_pX` play the same role as `n_products` / `n_products_pX` in industry summaries. `inspect_products` gains `percentiles` (default `[0.5, 1.0]`) and `coverage_thresholds` (default `[0.5, 0.8, 0.95]`) keyword parameters. `_build_products_summary` in `_industries.py` refactored to delegate to a new shared `_build_summary_table` helper in `_shared.py`. Separate `_style_categories_summary_table` added to `_style.py` (simpler border logic: thick borders between product blocks only). `_summary_row_format_func` generalised from `n_products` prefix check to `n_` prefix. `_percentile_label` moved from `_industries.py` to `_shared.py`.

- **2026-04-22 (session 7)**: Added `as_list: bool = False` and `table: str | None = None` keyword arguments to all five `get_*_codes` free functions and their `SUT` delegate methods. `as_list=True` returns a plain list (same dtype as data), omitting the txt column. `table="supply"/"use"` restricts code discovery to `sut.supply` or `sut.use` respectively; `None` (default) unions both. For `get_individual_consumption_codes` and `get_collective_consumption_codes`, `table="supply"` naturally returns empty (P31/P32 never appear in supply). Implemented via new `table` parameter on private helpers `_unique_column_values` and `_category_codes_for_esa`.

- **2026-04-22 (session 8)**: Added `use_products_summary` to `FinalUseInspection` / `FinalUseInspectionData`. Index: `(transaction, transaction_txt, category, category_txt, summary)` — one block per (transaction, category) pair. Statistics aggregate over products within each group. `inspect_final_uses` gains `percentiles` (default `[0.5, 1.0]`) and `coverage_thresholds` (default `[0.5, 0.8, 0.95]`) keyword parameters. New `_style_final_use_use_products_summary_table` added to `_style.py` (thick borders between transaction blocks, thin between category blocks within a transaction). Private helper `_build_final_use_use_products_summary` in `_final_uses.py` delegates to `_build_summary_table`.

- **2026-04-22 (session 9)**: `_apply_n_largest_filter` and `_apply_column_sort` in `_shared.py` both changed to operate on absolute values. Rationale: for diff/change columns, large negative values are as noteworthy as large positives — magnitude is the relevant ranking criterion.

- **2026-04-23 (session 10)**: Added `set_display_sort_ids_ascending(ascending=True)` to all 7 inspection classes and `TablesComparison`. Two cases handled: (1) wide-format tables where id values are columns — `id_columns=True` in `DisplayConfiguration` triggers column reordering in `_apply_display_config`; (2) tall-format tables where id is the outermost index level — `id_index_level=id_col` set at inspect-function construction time triggers `sort_index` in `_apply_display_config`. `_apply_display_config` restructured so id sorting runs before the `protected_tables` early-return, and protected-table styled properties in `_products.py`, `_industries.py`, and `_aggregates_nominal.py` now route through `_apply_display_config`. `sort_ids_ascending` and the two class-specific fields (`id_columns`, `id_index_level`) added to `DisplayConfiguration`; reset preserves `id_columns`/`id_index_level`, restores `sort_ids_ascending` to `True`. `SUTComparisonInspection` gets the method for API consistency but it has no visible effect (column names embed id values as suffixes, not raw id values).

- **2026-04-27 (session 11)**: Replaced "row-level" with "cell-level" throughout `inspect_sut_comparison` documentation — module docstring, `SUTComparisonData` field docstrings, and `table_description` df descriptions. No code or behaviour change.

- **2026-04-27 (session 12)**: Changed `set_display_index` semantics from additive (union-merge into existing level values) to replace (overwrite existing values for that level, or add if not present). Updated `_cfg_set_display_index` in `_display_config.py`, the `DisplayConfiguration.display_index` docstring, and CLAUDE.md.

- **2026-04-29 (session 13)**: Added `adjust_subtract_sut` (`sutlab/adjust/_subtract.py`) and `adjust_substitute_sut` (`sutlab/adjust/_substitute.py`). Subtract: negates adjustments price columns then sums via `_add_long_tables`; new rows appended as negated values; NaN treated as 0. Substitute: outer-merge replaces matching rows entirely with adjustments values (NaN = set to NaN, not treat-as-0); new rows appended; unmatched base rows unchanged. Both handle balancing targets with same semantics, exported from `__init__.py`, wired as SUT delegate methods.

- **2026-04-29 (session 14)**: Renamed all five `get_*_codes` functions (and their `SUT` delegate methods) to `get_codes_*`: `get_product_codes` → `get_codes_products`, `get_transaction_codes` → `get_codes_transactions`, `get_industry_codes` → `get_codes_industries`, `get_individual_consumption_codes` → `get_codes_individual_consumption`, `get_collective_consumption_codes` → `get_codes_collective_consumption`. Updated `sutlab/sut.py`, `sutlab/__init__.py`, `tests/test_sut.py`, `docs/sut_format.md`.

- **2026-04-29 (session 15)**: Added `aggregate_classification_products(sut, mapping, *, metadata=None, full_coverage=True, classification_name=None)` in new `sutlab/aggregate/` package. `mapping` is a DataFrame with `"from"` and `"to"` columns (many-to-one allowed). Aggregates by replacing product codes then summing price columns within `(id, product, transaction, category)` groups; result sorted by those keys. `full_coverage=True` (default) requires all data product codes to appear in `mapping["from"]`; `full_coverage=False` lets unmapped codes pass through. `metadata` replaces the products classification (full coverage) or is merged with existing rows for unmapped codes (partial coverage); `None` clears it. `classification_name` updates the product dimension's entry in `classification_names`; `None` sets it to NaN. `balancing_targets`, `balancing_config` both set to `None` — user must re-attach. `balancing_id` and `price_basis` preserved. `margin_products` remapped per Option B: for each `"to"` code, if all source products are margin products with the same `price_layer`, carry one row through (txt set to None); if mixed margin/non-margin or inconsistent layers, raise. Pass-through margin products kept unchanged. Wired as `SUT.aggregate_classification_products` delegate method.

- **2026-04-30 (session 16)**: Refactored `sutlab/aggregate/` to support all five `aggregate_classification_*` functions. New `_shared.py` holds all dimension-agnostic helpers: `_validate_mapping`, `_validate_full_coverage(mapping, dfs, col)`, `_validate_from_in_classification(mapping, cls_df, col, cls_label)`, `_validate_metadata_columns_simple(metadata, col)`, `_validate_transactions_metadata_columns(metadata, col)`, `_validate_no_passthrough_collision(mapping, dfs, col)`, `_aggregate_long_table(df, mapping, key_cols, col)`, `_aggregate_with_esa_filter(df, mapping, key_cols, col, trans_col, matching_trans)`, `_build_classification(old_cls_df, mapping, metadata, full_coverage, col)`, `_update_classification_names`, `_get_matching_trans`, `_require_transactions_classification`. `_products.py` refactored to use shared helpers. Four new functions added: `aggregate_classification_transactions` (`_transactions.py`), `aggregate_classification_industries` (`_industries.py`), `aggregate_classification_individual_consumption` (`_individual_consumption.py`), `aggregate_classification_collective_consumption` (`_collective_consumption.py`). All five functions: `balancing_config` cleared, `balancing_id` preserved. Products: `balancing_targets` pass through unchanged (no product column in targets). Transactions + category functions: `balancing_targets` aggregated with same mapping. Transactions: all rows in scope, all DataFrames (supply, use, targets) passed to validators. Category functions: aggregation scoped to ESA-relevant rows via `_aggregate_with_esa_filter`; `full_coverage` and passthrough-collision validation also scoped to ESA-filtered rows; ESA-filtered targets rows included in validation DataFrames; `transactions` classification required (raises if absent). Industries uses ESA codes P1/P2; individual_consumption uses P31; collective_consumption uses P32. `aggregate_classification_transactions` metadata validator requires `table` and `esa_code` columns (unlike simple col+txt validator used by others). All five wired as SUT delegate methods.
