...
class TestSortId:
    ...
    def test_sort_id_not_in_ids_raises(self, sut):
        with pytest.raises(ValueError, match="sort_id"):
            inspect_products(sut, "A", sort_id=9999)

    def test_sort_id_outside_ids_filter_raises(self, sut):
        # sort_id=2020 is valid in the full collection but excluded by ids filter
        with pytest.raises(ValueError, match="sort_id"):
            inspect_products(sut, "A", ids=2021, sort_id=2020)

    def test_balance_not_affected_by_sort_id(self, sut):
        unsorted = inspect_products(sut, "A").data.balance
        sorted_ = inspect_products(sut, "A", sort_id=2021).data.balance
        assert unsorted.index.tolist() == sorted_.index.tolist()

    def test_supply_products_sorted_within_product(self, sut_multi_cat):
        # supply_multi_cat rows for product A in 2021:
        #   "0100"/"X"=66, "0100"/"Y"=44, "0700"/""/=25
        # Natural sort order: 0100/X, 0100/Y, 0700/"", Total
        # After sort_id=2021 (desc): 0100/X=66, 0100/Y=44, 0700/""=25, Total — same (already desc)
        # Use 2020 to verify: 60, 40, 20 — same order, confirming no reorder here.
        # Use a product where sorting *changes* the order by checking use_products instead.
        result = inspect_products(sut_multi_cat, "A", sort_id=2021)
        detail = result.data.supply_products
        trans_vals = detail.index.get_level_values("transaction")
        cat_vals = detail.index.get_level_values("category")
        non_total = [(t, c) for t, c in zip(trans_vals, cat_vals) if t != ""]
        # Values in 2021: 0100/X=66, 0100/Y=44, 0700/""=25 — descending order
        assert non_total[0] == ("0100", "X")
        assert non_total[1] == ("0100", "Y")
        assert non_total[2] == ("0700", "")
        # Total stays last
        assert trans_vals[-1] == ""

    def test_use_products_sorted_within_product(self, sut_multi_cat):
        # use_multi_cat rows for product A:
        #   "2000"/"X": 2020=60, 2021=63
        #   "2000"/"Y": 2020=20, 2021=22
        #   "6001"/"":  2020=40, 2021=50
        # Flat sort by sort_id=2021 (desc): 63, 50, 22
        # → 2000/X, 6001/"", 2000/Y, Total
        result = inspect_products(sut_multi_cat, "A", sort_id=2021)
        detail = result.data.use_products
        trans_vals = detail.index.get_level_values("transaction")
        cat_vals = detail.index.get_level_values("category")
        non_total = [(t, c) for t, c in zip(trans_vals, cat_vals) if t != ""]
        assert non_total[0] == ("2000", "X")   # 63 — largest
        assert non_total[1] == ("6001", "")    # 50 — moved up from 3rd
        assert non_total[2] == ("2000", "Y")   # 22 — moved down from 2nd
        assert trans_vals[-1] == ""

    def test_use_products_total_stays_at_end(self, sut_multi_cat):
        result = inspect_products(sut_multi_cat, "A", sort_id=2021)
        detail = result.data.use_products
        trans_vals = detail.index.get_level_values("transaction")
        assert trans_vals[-1] == ""

    def test_use_products_distribution_inherits_sort_order(self, sut_multi_cat):
        sorted_ = inspect_products(sut_multi_cat, "A", sort_id=2021)
        assert (
            sorted_.data.use_products_distribution.index.tolist()
            == sorted_.data.use_products.index.tolist()
        )

    def test_use_products_growth_inherits_sort_order(self, sut_multi_cat):
        sorted_ = inspect_products(sut_multi_cat, "A", sort_id=2021)
        assert (
            sorted_.data.use_products_growth.index.tolist()
            == sorted_.data.use_products.index.tolist()
        )

    def test_price_layers_sorted_within_product_layer(self, sut_with_layers):
        # ava block: "2000" ava=[2,3], "3110" ava=[4,5]
        # Natural (pivot) order: "2000" < "3110" alphabetically → 2000 first
        # After sort_id=2021 (desc): 5 > 3 → "3110" first
        result = inspect_products(sut_with_layers, "A", sort_id=2021)
        pl = result.data.price_layers
        ava_block = pl[pl.index.get_level_values("price_layer") == "ava"]
        trans_vals = ava_block.index.get_level_values("transaction")
        non_total = [t for t in trans_vals if t != ""]
        assert non_total[0] == "3110"   # ava=5 in 2021 — largest
        assert non_total[1] == "2000"   # ava=3 in 2021
        assert trans_vals[-1] == ""     # Total stays last

    def test_price_layers_total_stays_at_end(self, sut_with_layers):
        result = inspect_products(sut_with_layers, "A", sort_id=2021)
        pl = result.data.price_layers
        ava_block = pl[pl.index.get_level_values("price_layer") == "ava"]
        assert ava_block.index.get_level_values("transaction")[-1] == ""

    def test_price_layers_distribution_inherits_sort_order(self, sut_with_layers):
        result = inspect_products(sut_with_layers, "A", sort_id=2021)
        assert (
            result.data.price_layers_distribution.index.tolist()
            == result.data.price_layers.index.tolist()
        )

    def test_price_layers_growth_inherits_sort_order(self, sut_with_layers):
        result = inspect_products(sut_with_layers, "A", sort_id=2021)
        assert (
            result.data.price_layers_growth.index.tolist()
            == result.data.price_layers.index.tolist()
        )

    def test_price_layers_rates_sorted_by_own_values(self, sut_with_layers):
        # Rates are independent of absolute values — they must be sorted by
        # their own sort_id column values, not inherited from price_layers.
        # ava block rates in 2021: "2000" ava-rate=3/22=0.136, "3110" ava-rate=5/49=0.102
        # So by rate descending: "2000" first (higher rate), "3110" second —
        # opposite of the absolute-value order ("3110" first in price_layers).
        result = inspect_products(sut_with_layers, "A", sort_id=2021)
        pl_rates = result.data.price_layers_rates
        ava_block = pl_rates[pl_rates.index.get_level_values("price_layer") == "ava"]
        trans_vals = ava_block.index.get_level_values("transaction").tolist()
