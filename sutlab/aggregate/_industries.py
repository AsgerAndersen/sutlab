# sutlab/aggregate/_industries.py — aggregate_classification_industries
...
def aggregate_classification_industries(
    ...
    *,
    ...
    """Return a new SUT with industry codes aggregated according to ``mapping``.

    Maps category codes on output (P1) and intermediate consumption (P2) rows
    from ``mapping['from']`` to ``mapping['to']``, then sums all price columns
    within each resulting group. Only rows whose transaction has ESA code P1 or
    P2 are affected; all other rows pass through unchanged.
    ...
    mapping : DataFrame
        Two-column DataFrame with ``'from'`` (original industry codes) and
        ``'to'`` (aggregated industry codes). Many-to-one is allowed;
        ``'from'`` must not contain duplicates or NaN.
    metadata : DataFrame or None, optional
        New industries classification table. Columns must be the actual
        category column name (e.g. ``'brch'``) and optionally that name with
        a ``'_txt'`` suffix. When ``full_coverage=True`` this replaces the
        existing industries classification entirely. When ``full_coverage=False``
        it is merged with existing rows for unmapped pass-through codes.
        ``None`` clears the industries classification.
    full_coverage : bool, default True
        If ``True``, every industry code (category code on P1/P2 rows) in
        supply, use, and (if present) balancing targets must appear in
    ...
    ValueError
        If ``full_coverage=True`` and any industry code in the data is absent
        from ``mapping['from']``.
    ValueError
        If the SUT already has an industries classification and any code in
        ``mapping['from']`` is absent from it.
    ValueError
        If ``metadata`` has incorrect columns.
    ValueError
        If ``full_coverage=False`` and any ``mapping['to']`` value is also
        a pass-through (unmapped) industry code in the data.
    """
    if not isinstance(sut, SUT):
        raise TypeError(f"sut must be a SUT instance, got {type(sut).__name__}.")
    if sut.metadata is None:
        raise ValueError(
            "sut.metadata is required to call aggregate_classification_industries. "
            "Provide a SUTMetadata with column name mappings."
        )
    _require_transactions_classification(sut, "aggregate_classification_industries")
    ...
    if old_cls is not None and old_cls.industries is not None:
        _validate_from_in_classification(
            mapping, old_cls.industries, cat_col, "industries"
        )
    ...
    old_industries_cls = old_cls.industries if old_cls is not None else None
    new_industries_cls = _build_classification(
        old_industries_cls, mapping, metadata, full_coverage, cat_col
    )

    if old_cls is None:
        new_cls: SUTClassifications | None = (
            SUTClassifications(industries=new_industries_cls)
            if new_industries_cls is not None
            else None
        )
    else:
