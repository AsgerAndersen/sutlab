...
### Module structure
- `sutlab/sut.py` — Core dataclasses: `SUT`, `SUTMetadata`, `SUTColumns`, `SUTClassifications`, `BalancingTargets`, `BalancingConfig`, `TargetTolerances`, `Locks`; and `set_balancing_id`, `set_balancing_targets`, `set_balancing_config`, `set_metadata`, `get_rows`, `get_product_codes`, `get_transaction_codes`, `get_ids`, `get_industry_codes`, `get_individual_consumption_codes`, `get_collective_consumption_codes`; pandas-style methods on `SUT` delegating to all public non-loader free functions including `write_to_separated_parquet`, `write_to_combined_parquet`, `write_to_separated_csv`, `write_to_combined_csv`, `write_to_separated_excel`, `write_to_combined_excel`
