...
- **2026-04-21** (session 3): Added `tables_description` as a `@property` to all 7 inspection result classes (`ProductInspection`, `IndustryInspection`, `FinalUseInspection`, `UnbalancedProductsInspection`, `UnbalancedTargetsInspection`, `SUTComparisonInspection`, `AggregatesNominalInspection`). Returns a DataFrame with columns `name` and `description` — one row per table — for easy discovery in Jupyter notebooks. Descriptions are hardcoded strings. `.columns` (per-table column descriptions) was considered and rejected: too much maintenance burden for marginal gain; column names in this codebase are already descriptive enough.
